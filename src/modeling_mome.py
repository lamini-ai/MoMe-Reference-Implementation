...
from src.lamini_index import LaminiIndex
from src.configuration_mome import LlamaMoMEConfig
...
def load_base_model_and_tokenizer(base_model_name, device):
    ...
    base_model = AutoModelForCausalLM.from_pretrained(base_model_name, token=token, cache_dir="cache", local_files_only=True,)
    tokenizer = AutoTokenizer.from_pretrained(base_model_name, token=token, cache_dir="cache", local_files_only=True,)
...
def set_mome_model_state_dict_for_inference(model, peft_model_state_dict, base_model_name):
    ...
    print(f"NEW STATE DICT {state_dict.keys()}")
    print(f"NEW STATE DICT DIMS {[v.shape for v in state_dict.values()]}")
...
def add_extra_lora_adapters_to_head(base_model_name: str, model: PreTrainedModel, config: LlamaMoMEConfig):
    ...
    for name, layer in model.named_modules():
        if is_tiny_lm_head_layer(base_model_name, name):
            ...
            recursive_setattr(model, name, LoraHeadAdaptor(layer, 64)) # TODO: THIS IS BRITTLE
    return model
...
def try_to_update_mlp(name, layer, model, config):
    ...
    recursive_setattr(model, name, LoraMLPAdaptor(layer, 64)) # TODO: THIS IS BRITTLE
